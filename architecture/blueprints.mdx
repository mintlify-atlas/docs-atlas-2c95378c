...
**Location**: `app/math_api/`

**Authentication**: Optional (rate-limited)

### Endpoints

<ParamField path="/api/math/validate" type="POST">
  **Validate mathematical expression**
  
  **Request Body**:
  ```json
  {
    "user_expression": "2*x + 3",
    "correct_expression": "3 + 2*x"
  }
  ```
  ...
  **Response**:
  ...
  {
    "is_equivalent": true,
    "normalized_user": "2*x + 3",
    "normalized_correct": "2*x + 3"
  }
  ```
...
| `/api/<resource>/<action>` | `/api/math/validate` | API endpoint |
...
@bp.route("/api/math/validate", methods=["POST"])
